...
Pesel::Pesel(const std::array<uint8_t, 11>& numbers)
: pesel_( numbers )
{
}
...
bool Pesel::validation_check_digit() const{
  static constexpr std::array<uint8_t, 10> weights
  {1, 3, 7, 9, 1, 3, 7, 9, 1, 3};
  ...
  auto modulo = sum % 10;
  return modulo == 0 && pesel_.at(10) == 0 ||
  10 - modulo == pesel_.at(10); 
}
...
bool operator==(const Pesel &pesel1, const Pesel &pesel2) {
  return std::equal(pesel1.pesel_.cbegin(), 
  pesel1.pesel_.cend(), 
